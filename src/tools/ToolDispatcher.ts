...
import {
	...
	collectPsdFiles,
	derivePrefabNameFromJson,
	derivePsdExportDir,
	normalizePsdStudioImportArgs,
} from "../utils/PsdStudioImport";
...
export class ToolDispatcher {
  ...
  static handleMcpCall(name, args, callback) {
		if (ToolDispatcher.isSceneBusy && (name === "save_scene" || name === "create_node")) {
			...
			case "generate_ui_from_psd":
				ToolDispatcher.generateUiFromPsd(args, callback);
				break;
  ...
  static _countDesignLogicBindings(logic) {
		const rules = Array.isArray(logic && logic.rules) ? logic.rules : [];
		const stats = {
		...
		rules.forEach((rule) => {
			const hasBinding =
				!!String((rule && rule.propertyName) || "").trim() ||
				!!String((rule && rule.dataKey) || "").trim() ||
				!!String((rule && rule.group) || "").trim() ||
				!!String((rule && rule.handlerName) || "").trim();
			if (hasBinding) {
				stats.bindingRules += 1;
			}
			if (String((rule && rule.handlerName) || "").trim()) {
				stats.handlerRules += 1;
			}
			if (String((rule && rule.group) || "").trim()) {
				stats.groupedRules += 1;
			}
		});
		return stats;
  }

  static _shouldAutoGenerateDesignLogic(logic) {
		const stats = ToolDispatcher._countDesignLogicBindings(logic);
		if (stats.totalRules <= 0) {
			return true;
		}
		if (stats.bindingRules <= 0) {
			return true;
		}
		return stats.bindingRules / stats.totalRules < 0.35;
  }

  static _buildRecommendedDesignImportArgs(spec, rawDocument, imageAssetPaths) {
		const logic = buildAutoDesignLogic(rawDocument, {
			prefabName: spec && spec.prefabName,
			imageAssetPaths: Array.isArray(imageAssetPaths) ? imageAssetPaths : [],
			imageAssetMap: (spec && spec.imageAssetMap) || {},
		});
		return {
			logic,
			importArgs: {
				jsonPath: spec && spec.jsonPath,
				prefabName: spec && spec.prefabName,
				prefabDir: spec && spec.prefabDir,
				imageAssetDirs: (spec && spec.imageAssetDirs) || [],
				...((spec && spec.imageAssetDirs && spec.imageAssetDirs[0])
					? { imageAssetDir: spec.imageAssetDirs[0] }
					: {}),
				imageAssetMap: (spec && spec.imageAssetMap) || {},
				...(spec && spec.rootPreset ? { rootPreset: spec.rootPreset } : {}),
				overwrite: !!(spec && spec.overwrite),
				logic,
			},
		};
  }
  ...
  static importDesignLayout(args, callback) {
		...
		let effectiveLogicSource = "request";
		let effectiveLogic = spec.logic;
		...
		try {
			...
			recommendedImport = ToolDispatcher._buildRecommendedDesignImportArgs(spec, rawDocument, imageAssetPaths);
			if (ToolDispatcher._shouldAutoGenerateDesignLogic(spec.logic)) {
				effectiveLogic = recommendedImport.logic;
				effectiveLogicSource = "auto-generated";
				Logger.info(
					`[import_design_layout] 检测到入参 logic 绑定信息不足，已切换为自动生成 logic。prefab=${spec.prefabPath}`,
				);
			}
			...
			normalizedLayout = applyDesignLayoutLogic(normalizedLayout, effectiveLogic);
		} catch (e) {
										...
										ToolDispatcher._generateAndAttachPrefabScript(
											...
											(scriptErr, scriptResult) => {
												callback(null, {
													...
													analysis,
													effectiveLogicSource,
													recommendedImportArgs: recommendedImport && recommendedImport.importArgs,
													created,
													script: scriptErr
														? {
																ok: false,
  ...
  static analyzeDesignLayout(args, callback) {
		...
		try {
			...
			const recommendedImport = ToolDispatcher._buildRecommendedDesignImportArgs(spec, rawDocument, imageAssetPaths);
			...
			const analysis = analyzeNormalizedDesignLayout(normalizedLayout);
			const logicReadiness = analyzeDesignLayoutLogicReadiness(normalizedLayout);
			const recommendedLayout = applyDesignLayoutLogic(
				normalizeDesignLayoutDocument(rawDocument, {
					assetOutputDir: spec.assetOutputDir,
					imageAssetPaths,
					imageAssetMap: spec.imageAssetMap,
					importGeneratedShapes: spec.importGeneratedShapes,
				}),
				recommendedImport.logic,
			);
			const recommendedAnalysis = analyzeNormalizedDesignLayout(recommendedLayout);
			const recommendedReadiness = analyzeDesignLayoutLogicReadiness(recommendedLayout);
			const providedLogicStats = ToolDispatcher._countDesignLogicBindings(spec.logic);
			...
			callback(null, {
				...
				logicReadiness,
				analysis,
				recommendedImportArgs: recommendedImport.importArgs,
				recommendedLogicReadiness: recommendedReadiness,
				recommendedAnalysis,
			});
		} catch (e) {
  ...
  static _resolveProjectInputPath(inputPath) {
		if (!inputPath || typeof inputPath !== "string") {
  ...
  static _getPsdStudioExecutablePath() {
		const candidates = [
			pathModule.resolve(__dirname, "../tools/psd2code-studio.exe"),
			pathModule.resolve(__dirname, "../../tools/psd2code-studio.exe"),
			pathModule.resolve(Editor.Project.path, "tools/psd2code-studio.exe"),
		];
  ...
  static _runPsdStudioExport(spec, callback) {
		const exePath = ToolDispatcher._getPsdStudioExecutablePath();
		if (!fs.existsSync(exePath)) {
			return callback(`找不到 psd2code-studio 可执行文件: ${exePath}`);
		}
		if (!fs.existsSync(spec.psdPath)) {
			return callback(`找不到 PSD 路径: ${spec.psdPath}`);
		}

		const stat = fs.statSync(spec.psdPath);
		const commandArgs = [
			stat.isDirectory() ? "-d" : "-f",
			spec.psdPath,
			"-t",
			"json",
		];
		if (spec.exportScreenshots) {
			commandArgs.push("-s");
		}
		if (spec.includeImageData) {
			commandArgs.push("-i");
		}

		Logger.info(`[generate_ui_from_psd] 执行导出: ${exePath} ${commandArgs.join(" ")}`);
		execFile(
			exePath,
			commandArgs,
			{
				cwd: pathModule.dirname(exePath),
				windowsHide: true,
			...
			(err, stdout, stderr) => {
				if (err) {
					...
					return callback(`psd2code-studio 执行失败: ${details || err.message}`);
				}
				callback(null, {
					exePath,
					commandArgs,
  ...
  static generateUiFromPsd(args, callback) {
		let spec;
		try {
			spec = normalizePsdStudioImportArgs(args, {
				projectRoot: Editor.Project.path,
			});
		} catch (e) {
			return callback(`PSD 导入参数无效: ${e.message}`);
		}

		const resolvedPsdPath = ToolDispatcher._resolveProjectInputPath(spec.psdPath);
		if (!fs.existsSync(resolvedPsdPath)) {
			return callback(`找不到 PSD 路径: ${resolvedPsdPath}`);
		}
		spec = {
			...spec,
		...
		const psdFiles = collectPsdFiles(spec.psdPath);
		if (psdFiles.length === 0) {
			return callback(`未在目标路径下找到 PSD 文件: ${spec.psdPath}`);
		}

		ToolDispatcher._runPsdStudioExport(spec, (exportErr, exportResult) => {
			if (exportErr) {
				return callback(exportErr);
			}

			const usedPrefabNames = new Set<string>();
			...
			psdFiles.forEach((psdFile, fileIndex) => {
				const exportDir = derivePsdExportDir(psdFile);
				const jsonFiles = collectGeneratedJsonFiles(exportDir);
				jsonFiles.forEach((jsonPath, jsonIndex) => {
					artifacts.push({
						psdFile,
						exportDir,
						jsonPath,
						prefabName: derivePrefabNameFromJson(jsonPath, {
							usedNames: usedPrefabNames,
							fallbackIndex: fileIndex + jsonIndex + 1,
						}),
					});
				});
			});
			...
			if (artifacts.length === 0) {
				return callback(`psd2code-studio 已执行，但未发现可导入的 JSON 输出: ${spec.psdPath}`);
			}
			...
			const runNext = (index) => {
				if (index >= artifacts.length) {
					if (imported.length === 0) {
						const preview = failed.slice(0, 3).map((item) => `${item.jsonPath}: ${item.error}`).join(" | ");
						return callback(`PSD 导入失败，未成功生成任何页面。${preview ? ` ${preview}` : ""}`);
					}
					return callback(null, {
						ok: failed.length === 0,
						psdPath: spec.psdPath,
						prefabDir: spec.prefabDir,
						imageAssetDirs: spec.imageAssetDirs,
						export: exportResult,
						psdFiles,
						artifacts,
				...
				let rawDocument;
				try {
					rawDocument = JSON.parse(fs.readFileSync(artifact.jsonPath, "utf8"));
				} catch (e) {
					failed.push({
						...artifact,
				...
				const logic = buildAutoDesignLogic(rawDocument, {
					prefabName: artifact.prefabName,
				});
				const importArgs = {
					...
					prefabDir: spec.prefabDir,
					imageAssetDirs: spec.imageAssetDirs,
					rootPreset: spec.rootPreset,
					overwrite: spec.overwrite,
					logic,
				};

				ToolDispatcher.importDesignLayout(importArgs, (importErr, importResult) => {
					if (importErr) {
						failed.push({
							...artifact,
							logic,
