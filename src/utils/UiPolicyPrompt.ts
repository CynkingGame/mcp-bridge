...
export function buildUiPolicyWorkflowGuide(policyInput?: Partial<UiPolicyConfig> | null): string {
	...
	return [
		...
		"8. 生成 prefab 后，默认同步生成并挂载同名脚本；脚本中要为动态数据和交互组件生成 `@property` 与绑定代码。",
...
export function buildDesignImportPlannerPrompt(
	...
	return [
		...
		"4. 如果 `logicReadiness.requiresExplicitLogic=true`，先补全逻辑命名与逻辑分组，不要直接导入。",
		"5. 如果图片节点仍未绑定正式素材，则优先补齐 `imageAssetMap`；不要直接让执行层猜。",
		"6. 执行 `import_design_layout` 时默认就是正式素材严格模式；素材缺失或逻辑命名不足时应直接失败，而不是偷偷降级。",
		"7. 导入后使用 `validate_ui_prefab` 校验根节点与按钮锚点。",
		"",
		"禁止事项：",
