...
export function buildUiPolicyWorkflowGuide(policyInput?: Partial<UiPolicyConfig> | null): string {
	...
	return [
		...
		"- JSON 中的 base64 图片默认只作为兜底数据，除非明确需要，否则不要开启 `importEmbeddedImages`。",
		"",
		"## 推荐流程",
		"1. `open_prefab` 或进入目标场景。",
		"2. `create_node` 时优先传 `uiPreset`；按钮默认遵循中心锚点规则。",
		"3. 如果已有设计 JSON，优先调用 `import_design_layout`，并同时传入 `jsonPath`、`prefabName`、`imageAssetDir`；只有正式素材缺失时才考虑 `importEmbeddedImages: true`。",
